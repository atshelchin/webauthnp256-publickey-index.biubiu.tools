...
`.github/workflows/ci.yml`:push/PR 到 main 时两个 job 作为合并门禁(分支保护应同时要求两者绿):

- `deno`:`deno check --frozen deno/index.ts build.ts scripts/deploy.ts`(--frozen 拒绝过期 deno.lock,与生产 systemd 启动一致)+ `deno task test`。Deno 钉到 v2.7.x(与本文档验证环境同 minor)
- `worker`:`npm ci` + `npm test`(vitest + miniflare,覆盖资金 DO 与 D1 队列路径)

**刻意不含**(理由写在 ci.yml 头注释里):链上 E2E(PRIVATE_KEY 门控自动跳过)、`deno lint`(3 个 require-await 待清理)、worker/ 的独立 `tsc --noEmit`(08 P2-15)。注意 CI 绿 ≠ 已部署——两条部署链路均为手动(见 05)。
