...
创建一条公钥记录。需要先获取 challenge, 用 WebAuthn `navigator.credentials.get()` 签名后提交。
...
| signature | string | 是 | WebAuthn assertion 签名 (raw r\|\|s 格式, hex) |
| authenticatorData | string | 是 | WebAuthn assertion 的 authenticatorData (base64url) |
| clientDataJSON | string | 是 | WebAuthn assertion 的 clientDataJSON (base64url) |
| name | string | 否 | passkey 的显示名称 (如 "我的 MacBook"), 默认空字符串 |

**签名验证方式**: 服务端按 WebAuthn 标准验证 — 从 clientDataJSON 中提取 challenge 校验, 然后验证签名是否匹配 `authenticatorData || SHA-256(clientDataJSON)` (P256 + SHA256)。authenticatorData 和 clientDataJSON 仅用于验证, 不存入数据库。
...
### POST /api/backup (需要 Admin Key)

手动触发备份。服务启动后 10 秒自动执行首次备份, 之后每小时自动备份一次。

**请求头**: `X-Admin-Key: <你的ADMIN_API_KEY>`
...
{
  "success": true,
  "url": "https://r2-public-domain/backups/hourly/backup-2026-03-21T10-00-00-000Z.db"
}
```

**错误响应**: `401` - 缺少或错误的 Admin Key

**备份策略**:
- 每次备份同时写入 `hourly/` 和 `daily/` 两个目录
- `hourly/` 保留最近 24 份 (覆盖 1 天回滚窗口)
- `daily/` 保留最近 7 份 (同一天多次备份只覆盖当天文件)
- 备份完成后通过 Telegram 通知管理员

R2 目录结构:
```
backups/
  hourly/backup-2026-03-21T10-00-00-000Z.db   ← 保留最近 24 个
  daily/backup-2026-03-21.db                   ← 保留最近 7 个
```

**调用示例**:
```bash
curl -X POST -H "X-Admin-Key: your-key" https://webauthnp256-publickey-index.biubiu.tools/api/backup
```

---

### POST /api/restore (需要 Admin Key)
...
```json
{
  "url": "https://r2-public-domain/backups/daily/backup-2026-03-21.db"
}
```
...
**调用示例**:
```bash
curl -X POST -H "X-Admin-Key: your-key" -H "Content-Type: application/json" \
  -d '{"url":"https://..."}' \
  https://webauthnp256-publickey-index.biubiu.tools/api/restore
```
...
2. navigator.credentials.get({ challenge })   → assertion (signature + authenticatorData + clientDataJSON)
3. POST /api/create                           → 201 Created
   Body: { rpId, credentialId, publicKey, challenge, signature, authenticatorData, clientDataJSON, name }
...
# 管理接口认证 (backup/restore 必需)
ADMIN_API_KEY=<随机生成一个强密钥>
```
