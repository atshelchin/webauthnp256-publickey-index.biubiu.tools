...
| initialCredentialId | string | 是 | 初始凭证 ID (首次注册时等于 credentialId, 密钥轮换时指向根凭证) |
| metadata | string | 是 | 附加元数据 (hex) |
...
部署流程: 上传源码 → 符号链接切换 → systemd 重启 → 健康检查。首次部署自动创建用户、目录、systemd 服务。
